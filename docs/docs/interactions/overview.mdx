...
The experimental `MotiPressable` component lets you animate based on `pressed` and `hovered` interactions, without triggering any re-renders.
...
Make sure you've also followed the [Installation steps](/installation) for `moti`.
...
For example, you have a dropdown menu whose `pointerEvents` should be `none` when it's container isn't hovered.
